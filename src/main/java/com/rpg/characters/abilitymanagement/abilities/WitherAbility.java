...
public class WitherAbility implements Ability {
    @Override
    public AbilityOutcome execute(AbilityContext ctx) {
        Player p = ctx.player();
        Enemy e = ctx.target();

        var meta = AbilityRegistry.getAbilityMetadata(AbilityId.WITHER);

        // Ignore defender's defense by passing 0 to the calculator
        int damage = p.calculateDamageDoneByPlayer(0, 1, AttackBonusType.ADDITIVE);
        ...
        return new AbilityOutcome(
                ...
                e.getHealthPoints(),
                List.of("You use " + meta.name() + " and ignore the opponent's defense!",
