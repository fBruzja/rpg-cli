...
public class ElectrocuteAbility implements Ability {
    @Override
    public AbilityOutcome execute(AbilityContext ctx) {
        Player p = ctx.player();
        Enemy e = ctx.target();

        var meta = AbilityRegistry.getAbilityMetadata(AbilityId.ELECTROCUTE);
        int damage = p.calculateDamageDoneByPlayer(e.getDefense(), 10, AttackBonusType.ADDITIVE);
